            ...
            for dtype in data_types:
                ...
                if not missing and not force:
                    ...
                    if self.cache.is_s3():
                        ...
                        if force or not self.cache.s3_prefix_exists(s3_prefix):
            ...
            if not missing and not force:
                if self.cache.is_s3():
                    for day in self.cache._date_range(from_date, to_date):
                        ...
                        if bar_path.exists():
                            s3_key = self.cache.s3_tardis_bar_key(exchange, symbol, freq, day)
            ...
            for rstart, rend in ranges:
                ...
                for day in days:
                    try:
                        ...
                        # Only ensure raw for days where we actually need to build the bar.
                        # Prevents re-downloading raw (quota risk) when bars already exist (e.g. keep_raw=False past ranges).
